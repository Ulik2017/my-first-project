...
async function loadCountries(){
  try {
    ...
    const countries = data.map(x=>x?.name?.common).filter(Boolean).sort((a,b)=>a.localeCompare(b));
    countryList.innerHTML = countries.map(c=>`<option value="${c}"></option>`).join('');
  } catch (e) {
    console.warn('Country lookup gagal dimuat', e);
  }
}
...
function renderCustomerTable(){ customerTable.querySelector("tbody").innerHTML = state.customers.map(c=>`<tr><td>${c.name}</td><td>${c.pic}</td><td>${c.country}</td><td>${c.state}</td><td>${c.address||""}</td><td>${c.email}</td><td>${c.phone}</td><td>${c.website}</td><td><button onclick="editCustomer('${c.id}')">Edit</button></td></tr>`).join(""); }
...
window.editCustomer=(id)=>{const c=byId(state.customers,id);customerId.value=c.id;customerName.value=c.name;customerPic.value=c.pic;customerCountry.value=c.country;customerState.value=c.state;customerEmail.value=c.email;customerPhone.value=c.phone;customerWebsite.value=c.website;customerAddress.value=c.address||"";};
...
function renderAll(){renderSalesOptions();renderCustomerOptions();renderProgressOptions();
renderProvinceOptions();
loadCountries();renderSalesTable();renderCustomerTable();renderQuotationTable();renderKanban();}
